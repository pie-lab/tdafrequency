...
names(serps_all_means) <- PRONOUNS_LIST
serps_all_means
...
Now that we have compared the raw means in the prior step, we create a table of the zscores of the 10-year averages for each TDA + pronoun ngram. These are the data posted on Dataverse, as shown below.
...
This csv file (above) has been posted publicly on Dataverse (see doi). All subsequent analyses of the search engine results data are based on this file, including the database of search engine estimates (see here).

*ADD HMTL LINK TO THE SENTENCE ABOVE*
...
### 3. Correlations between the 5 TDA + pronoun vectors
...
The table below shows the correlations between pronoun pairs with the lower and upper bounds of the 95% confidence intervals.
...
Here, we find the frequencies index by finding the average of the z-scores across all 5 TDA + pronoun pairs for each TDA *after* removing the max z-score for each TDA. In other words, find the average among all but the largest z-score for each TDA. This approach is used to reduce the skew introduced in a few cases where the pairing of a specific TDA + pronoun is part of a song, book, famous individual, etc.

Output from the following section of code is used to populate the serps column of the database of frequencies provided on this website.

*ADD URL LINK WHEN COMPLETED*
...
Once loaded, we derive the means for each term + pronoun ngram across the 10 year period from 2010 to 2019. These averages are then combined in a new data frame.
...
Calculate the overall means for each of the TDA + pronoun vectors. In other words, find the means for all the TDA + person ngrams, TDA + man ngrams, woman, girl, and boy. Then compare the means with t-tests. This is repeated using holm and bonferroni corrections.

```{r, warning=FALSE, label = "analyses_ngrams_means"}
# drop the column of TDA labels
ngrams_avgfreq <- subset(ngrams_all_avgfreq, select = -c(term))
means_vector <- c()
...
names(means_vector) <- PRONOUNS_LIST
means_vector
...
This csv file (above) has been posted publicly on Dataverse (see doi). All subsequent analyses of the ngrams data are based on this file, including the database of ngram estimates (see here).
...
cor_ngrams_ci <- tibble(
  pronoun1 = c("person", "person", "person", "person", "woman", 
             "woman", "woman", "man", "man", "girl"),
  pronoun2 = c("woman", "man", "girl", "boy", "man", 
             "girl", "boy", "girl", "boy", "boy"),
...
Output from the following section of code is used to populate the ngram column of the database of frequencies provided on this website.
