...
As discussed in the pre-registration, we evaluated the correlation between the search engine frequency estimates presented here and the 2010 estimates reported in Roivainen (2013). This is done using z-scored versions of the raw counts provided by Roivainen (personal correspondence).

```{r, warning=FALSE, label = "roivainen_comparison"}
rm(list=ls())
roivainen <- read_csv(here("data/other/serps_roivainen_comparison.csv"))
